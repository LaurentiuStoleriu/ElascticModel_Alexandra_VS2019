...
int main(void)
{
	...
	char cale_in[100] = "E:\\Stoleriu\\C\\2d_elastic\\ElascticModel_Alexandra_VS2019";
	...
	for (int i = 0; i < DIM; i++)
	{
		if (abs(sol[6 * i]) < 0.01 && abs(sol[6 * i + 2]) < 0.01)
			central = i;
		//cout << central << '\n';
		if ( (sol[6 * i] < x_minim / 2.0 + 0.5) && (sol[6 * i] > x_minim / 2 - 0.5) && (abs(sol[6 * i + 2]) < 0.01) )
			molec_2 = i;
	}
	...
	while (!ech)
	{
		...
		if (count % 2 == 1)
		{
			// ************************** AFISARE *************************************
			char fisier_out[300];
			char cale_out[150] = "E:\\Stoleriu\\C\\2d_elastic\\ElascticModel_Alexandra_VS2019\\Hexagon_19";
			sprintf(fisier_out, "%s\\pas_%d.dat", cale_out, count);
...
void update_anch_init_pos(int dot)
{
	...
	anch[dot][2].x_init = x_centru - r[dot] * sinPi3; // sin(Pi / 3);
	anch[dot][2].y_init = y_centru - r[dot] * cosPi3; // cos(Pi / 3);

	anch[dot][3].x_init = x_centru + r[dot] * sinPi3; // sin(Pi / 3);
	anch[dot][3].y_init = y_centru - r[dot] * cosPi3; // cos(Pi / 3);

	anch[dot][4].x_init = x_centru - r[dot] * sinPi3; // sin(Pi / 3);
	anch[dot][4].y_init = y_centru + r[dot] * cosPi3; // cos(Pi / 3);

	anch[dot][5].x_init = x_centru + r[dot] * sinPi3; // sin(Pi / 3);
	anch[dot][5].y_init = y_centru + r[dot] * cosPi3; // cos(Pi / 3);
